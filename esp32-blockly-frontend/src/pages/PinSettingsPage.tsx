...
import { usePinPresetStore, SERVO_TYPE_DEFAULTS, type ServoType } from '@/stores/pinPresetStore';
  ...
  return (
    <div className="min-h-screen bg-[#0D1117]">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* 注意事項 */}
        <Card className="mt-6 border-amber-600 bg-amber-900/20">
          <CardContent className="pt-4">
            <p className="text-sm text-amber-200">
              <strong>{t('pinSettings.notice')}</strong> {t('pinSettings.noticeText')}
            </p>
          </CardContent>
        </Card>
      ...
      <Dialog open={newPresetDialogOpen} onOpenChange={setNewPresetDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('pinSettings.createNewPreset')}</DialogTitle>
            <DialogDescription>
              {t('pinSettings.createNewPresetDesc')}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Label htmlFor="preset-name">{t('pinSettings.presetName')}</Label>
            <Input
              id="preset-name"
              value={newPresetName}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setNewPresetDialogOpen(false)}>
              {t('common.cancel')}
            </Button>
