...
// ============================================================
// Submissions proxy (ML30 digicode-class-server)
// ============================================================

// GET /api/classes/submissions/my — 生徒自身の submission 一覧
classes.get('/submissions/my', async (c) => {
  try {
    const { userId } = c.get('user');
...
// GET /api/classes/submissions/:id — submission 詳細
classes.get('/submissions/:id', async (c) => {
  try {
    const { userId } = c.get('user');
    const id = parseInt(c.req.param('id'));
    if (isNaN(id)) return c.json({ error: '無効なIDです' }, 400);
...
// PUT /api/classes/submissions/:id — 答案保存
classes.put('/submissions/:id', async (c) => {
  try {
    const { userId } = c.get('user');
    const id = parseInt(c.req.param('id'));
    if (isNaN(id)) return c.json({ error: '無効なIDです' }, 400);

    // 先にsubmission を取得して自分のものかチェック
...
// POST /api/classes/submissions/:id/submit — 提出
classes.post('/submissions/:id/submit', async (c) => {
  try {
    const { userId } = c.get('user');
    const id = parseInt(c.req.param('id'));
    if (isNaN(id)) return c.json({ error: '無効なIDです' }, 400);
