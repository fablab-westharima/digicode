...
| 零件 | 類別 | 跳轉 |
|---|---|---|
| 超音波距離 (HC-SR04) | 感測器 (數位) | [→](#超音波距離感測器-hc-sr04) |
| 溫濕度 (DHT11 / DHT22) | 感測器 (1-wire) | [→](#溫濕度感測器-dht11-dht22) |
| 循跡 (QTR-8) | 感測器 (類比) | [→](#qtr-循跡感測器-8通道) |
| 加速度・陀螺儀 (MPU6050) | 感測器 (I2C) | [→](#加速度陀螺儀感測器-mpu6050) |
| 氣壓・溫濕度 (BME280 / BMP280) | 感測器 (I2C) | [→](#氣壓溫濕度感測器-bme280-bmp280) |
| ToF 距離 (VL53L0X) | 感測器 (I2C) | [→](#tof-距離感測器-vl53l0x) |
| 磁性編碼器 (AS5600) | 感測器 (I2C) | [→](#磁性編碼器-as5600) |
| 伺服 (SG90 等) | 致動器 | [→](#伺服馬達-sg90-等) |
| 直流馬達 (透過 L298N) | 致動器 | [→](#直流馬達-透過馬達驅動器) |
| NeoPixel (WS2812B) | 致動器 | [→](#neopixel-led-ws2812b) |
| LCD (16x2 + PCF8574) | 通訊模組 (I2C) | [→](#i2c-lcd-顯示器) |
| TFT (ST7789 / ILI9341) | 通訊模組 (SPI) | [→](#tft-顯示器-spi) |
| RFID (RC522) | 通訊模組 (SPI) | [→](#rfid-讀卡機-rc522) |
| IR 接收 (VS1838B) | 通訊模組 | [→](#ir-接收模組-vs1838b) |
| DFPlayer Mini (MP3) | 通訊模組 (UART) | [→](#dfplayer-mini-mp3) |
| ESP32-CAM / XIAO Sense | 攝影機 | [→](#攝影機模組連接) |
| CAN Bus (SN65HVD230) | 通訊 | [→](#can-bus-連接-twai) |

---

## ESP32 腳位速查表

ESP32 開發板有各種類型,但共通的前提知識:

| 腳位 | 用途 | 注意 |
|---|---|---|
| **GPIO 0-39** | 數位 I/O | — |
| **GPIO 32-39** | 僅類比輸入 (ADC1) | ADC2 在 WiFi 使用時衝突 |
| **GPIO 0, 2** | 內建 LED (依開發板而異) | — |
| **GPIO 1, 3** | UART (序列) | 用於燒錄,避免使用 |
| **GPIO 6-11** | 僅 Flash 記憶體 | 無法使用 |
| **GPIO 34-39** | 僅輸入 | 無上拉電阻 |
| **3.3V** | 感測器電源 | 最大 200mA |
| **5V** | USB 5V (部分開發板) | 馬達用建議外部電源 |
| **GND** | 接地 | 必須共同接地 |

> 💡 **I2C 標準腳位**: SDA = GPIO 21、SCL = GPIO 22 (ESP32 預設)
> 💡 **SPI 標準腳位**: SCK = GPIO 18、MOSI = GPIO 23、MISO = GPIO 19 (硬體 SPI)
...
- [ESP32 燒錄指南](./04-program-setup-esp32.md) — 燒錄方式詳細
