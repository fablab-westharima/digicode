...
export const AI_SYSTEM_PROMPTS: Record<AiLanguage, PromptTemplates> = {
  ja: {
    blockGen: {
      ...
      prohibitions: '以下のことを厳守してください：\n- 上記リスト外の type を使ってはいけません\n- 自然言語で返答してはいけません\n- 会話で確定した仕様に従い、XML を完成させてください（再度の質問は不要）\n- コードフェンス（```）で XML を囲んではいけません\n- ★ 印の field（SSID・パスワード・APIキー・トークン・ブローカー等）は XML に含めてはいけません。省略すればブロックがデフォルト値（your_ssid 等）を表示し、ユーザーが直接入力します\n- ★ 印以外の field でも、WiFi URL・MQTT ブローカーアドレス・HTTP エンドポイント等の実値を推測・生成してはいけません\n- ★ ble_uart_on_receive の HANDLER 内では、受信値を読み取る Blockly ブロックは存在しません。variables_get で \'receivedData\' / \'bleMessage\' / \'data\' 等の変数を作成して受信値と比較することは禁止です。受信は「合図」として扱い、固定動作（LED フラッシュ・固定文字列の送信・モーター起動など）のみ HANDLER 内に配置してください。受信値による条件分岐をユーザーが要求した場合は、XML を返さず会話で「現在 Blockly では受信値の読み取りはサポートされていません」と伝えてください',
  ...
  'zh-TW': {
    blockGen: {
      ...
      prohibitions: '請嚴格遵守以下規則：\n- 只能使用提供列表中的積木類型\n- 不得以自然語言回應\n- 依照對話中確定的規格完成 XML（無需再次詢問）\n- 不得將 XML 包在程式碼圍欄（```）中\n- 絕對不能在 XML 中包含 ★ 欄位（SSID、密碼、API 金鑰、Token、Broker 等）。省略這些欄位後，積木會顯示安全的預設值（如 your_ssid），供使用者直接填入\n- 不得推測或生成 WiFi URL、MQTT Broker 地址、HTTP 端點或其他使用者特定連線資訊的實際值\n- ★ 在 ble_uart_on_receive 的 HANDLER 內，沒有可以讀取接收值的 Blockly 積木。禁止使用 variables_get 建立 "receivedData"、"bleMessage"、"data" 等變數來與接收值比較。請將「接收」視為觸發訊號，HANDLER 內僅放置固定動作（LED 閃爍、發送固定字串、啟動馬達等）。若使用者明確要求依接收值進行條件分支，不要產生 XML——改在對話中說明「目前 Blockly 不支援讀取接收值」。',
