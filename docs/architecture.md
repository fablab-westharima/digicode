...
┌────────────────────────────┐    ┌──────────────────────────┐
│    フロントエンド           │    │      ESP32デバイス       │
│  (React + Blockly)         │    │  (Arduino C++/MicroPython)│
│  localhost:5173            │    └──────────────────────────┘
└────────────┬───────────────┘
             │
             │ HTTP/REST API
   ...
   v                    v                         v
┌──────────────┐  ┌────────────────┐   ┌───────────────────┐
│バックエンドAPI│  │コンパイルサーバー│   │  Cloudflare D1   │
│ (Hono +      │  │ (Node.js +     │   │   (SQLite)       │
│  Workers)    │  │  Arduino CLI)  │   │                  │
│ localhost:8787│  │ localhost:3001 │   │                  │
└──────────────┘  └────────────────┘   └───────────────────┘
```
...
コンパイルサーバー
  ↓ multicast-dns
ESP32デバイス（_digicode._tcp.local）
  ↓ デバイス情報返却
フロントエンド（デバイス選択）
  ↓ .binファイルアップロード
ESP32デバイス（/doUpdate）
